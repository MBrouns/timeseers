...
class Constant(TimeSeriesModel):
    def __init__(
        self, name: str = None, lower=0, upper=1, pool_cols=None, pool_type="complete"
    ):
        ...
        self.name = (
            name
            or f"Constant(lower={self.lower}, upper={self.upper}, pool_cols='{self.pool_cols}', "
    ...
    def definition(self, model, X, scale_factor):
        ...
        with model:
            if self.pool_type == "partial":
                ...
                mu_c = pm.Uniform(
                    self._param_name("mu_c"), lower=self.lower, upper=self.upper
                )
