...
def test_minmax_scaler_np(array):
    ...
    np.testing.assert_allclose(
        scaler.fit(array).transform(array), scaler.fit_transform(array)
    )
...
def test_minmax_scaler_series(series):
    ...
    np.testing.assert_allclose(
        scaler.fit(series).transform(series), scaler.fit_transform(series)
    )
    np.testing.assert_allclose(
        series, scaler.inv_transform(scaler.transform(series)), rtol=1e-06
    )
